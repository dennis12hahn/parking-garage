package com.example.parkinggarage.controller.attendant.tickets_and_receipts;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (docType.equals("Ticket")) {
            for (int i = 0; i < doc.getTicketInfo().size(); i++) {
                content.append(doc.getTicketInfo().get(i) + "\n");
            }
        }

        if (docType.equals("Receipt")) {
            for (int i = 0; i < doc.getReceiptInfo().size(); i++) {
                content.append(doc.getReceiptInfo().get(i) + "\n");
            }
        }
